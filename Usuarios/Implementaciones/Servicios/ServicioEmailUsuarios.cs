...
namespace Usuarios.Implementaciones.Servicios
{
    public class ServicioEmailUsuarios
    {
        ...
        public async Task EnviarCorreoNuevoReporte(string destinatario, string descripcion, string lugar, string nombreReportante)
        {
            // Cargar la plantilla HTML desde un archivo
            var basePath = AppContext.BaseDirectory;
            ...
            mensaje.To.Add(destinatario);
            mensaje.Subject = "Nuevo reporte de falla";
            mensaje.IsBodyHtml = true;
            mensaje.Body = html;
            mensaje.ReplyToList.Add(new MailAddress(_configuracion.User));
            mensaje.Priority = MailPriority.High;
        ...
        public async Task EnviarCorreoRecepcionReporte(string destinatario, string descripcion, string lugar, string nombreReportante)
        {
            // Cargar la plantilla HTML desde un archivo
            var basePath = AppContext.BaseDirectory;
            var templatePath = Path.Combine(basePath, "Templates", "recepcion_reporte_plantilla.html");
            string htmlTemplate = File.ReadAllText(templatePath);

            // Reemplazar el marcador de posición en la plantilla con el OTP
            string html = htmlTemplate.Replace("{{DESCRIPCION}}", descripcion).Replace("{{LUGAR}}", lugar).Replace("{{NOMBRE_REPORTANTE}}", nombreReportante);
            ...
            mensaje.To.Add(destinatario);
            mensaje.Subject = "Recepción de reporte de falla";
            mensaje.IsBodyHtml = true;
            mensaje.Body = html;
            mensaje.ReplyToList.Add(new MailAddress(_configuracion.User));
            mensaje.Priority = MailPriority.High;
        ...
        public async Task EnviarCorreoSolucionReporte(string destinatario, string descripcion, string lugar, string nombreReportante)
        {
            // Cargar la plantilla HTML desde un archivo
            var basePath = AppContext.BaseDirectory;
            var templatePath = Path.Combine(basePath, "Templates", "solucion_reporte_plantilla.html");
            string htmlTemplate = File.ReadAllText(templatePath);
            ...
            mensaje.To.Add(destinatario);
            mensaje.Subject = "Solución a reporte de falla";
            mensaje.IsBodyHtml = true;
            mensaje.Body = html;
            mensaje.ReplyToList.Add(new MailAddress(_configuracion.User));
            mensaje.Priority = MailPriority.High;
