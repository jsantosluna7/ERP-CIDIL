...
namespace Usuarios.Implementaciones.Repositorios
{
    public class RepositorioReporteFalla : IRepositorioReporteFalla
    {
        ...
        public async Task<Resultado<ReporteFalla?>> ActualizarReporte(int id, ActualizarReporteFallaDTO actualizarReporteFallaDTO)
        {
            ...
            // Solo se envía correo si el usuario existe. Lugar es nullable en el modelo.
            if (usuario != null)
            {
                string lugarReporte = reporteExiste.Lugar ?? "";
                string nombreCompleto = $"{usuario.NombreUsuario} {usuario.ApellidoUsuario}";

                if (reporteExiste.Estado == 2)
                {
                    await _servicioEmail.EnviarCorreoRecepcionReporte(usuario.CorreoInstitucional, reporteExiste.Descripcion, lugarReporte, nombreCompleto); //Correo para confirmar el reporte de falla al usuario que lo reporto.
                }

                if (reporteExiste.Estado == 3)
                {
                    await _servicioEmail.EnviarCorreoSolucionReporte(usuario.CorreoInstitucional, reporteExiste.Descripcion, lugarReporte, nombreCompleto); //Correo para avisar solucion al reporte de falla al usuario que lo reporto.
                }
            }
        ...
        public async Task<Resultado<ReporteFalla?>> CrearReporte(CrearReporteFallaDTO crearReporteFallaDTO)
        {
            try
            {
                ...
                string lugarNuevo = Reporte.Lugar ?? "";
                string nombreReportante = usuarioReporta != null
                    ? $"{usuarioReporta.NombreUsuario} {usuarioReporta.ApellidoUsuario}"
                    : "";

                foreach (var usuarios in usuario)
                {
                    await _servicioEmail.EnviarCorreoNuevoReporte(usuarios.CorreoInstitucional, Reporte.Descripcion, lugarNuevo, nombreReportante); //Correo para que se le envie el reporte de falla a todos los administradores.
                }
        ...
        public async Task<Resultado<List<ReporteFalla?>>> GetReporteFalla()
        {
            ...
            // Se convierte a List<ReporteFalla?> para coincidir con el tipo de retorno (CS8620).
            return Resultado<List<ReporteFalla?>>.Exito(resultado.Cast<ReporteFalla?>().ToList());
        }
