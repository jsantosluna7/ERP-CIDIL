    ...
    public class RepositorioPrestamosEquipo : IRepositorioPrestamosEquipo
    {
        ...
        public async Task<PrestamosEquipo?> Crear(CrearPrestamosEquipoDTO crearPrestamosEquipoDTO)
        {
            ...
            if (crearPrestamosEquipoDTO.IdEstado == 1) // Si el estado es "Aprobado"
            {
                ...
                {
                    string cantidadStr = pEquipo.Cantidad?.ToString() ?? "0";
                    await _servicioEmail.EnviarCorreoAprobacionEquipos(usuario.CorreoInstitucional, usuario.NombreUsuario, cantidadStr, usuario.ApellidoUsuario, inventario.Nombre, fechaFormateadaInicio, fechaFormateadaFinal);
                }
            }
            ...
            { 
                ...
                {
                    string cantidadStr = pEquipo.Cantidad?.ToString() ?? "0";
                    string comentario = crearPrestamosEquipoDTO.ComentarioAprobacion ?? "";
                    await _servicioEmail.EnviarCorreoRechazoEquipos(usuario.CorreoInstitucional, usuario.NombreUsuario, cantidadStr, usuario.ApellidoUsuario, inventario.Nombre, comentario, fechaFormateadaInicio, fechaFormateadaFinal);
                }
            }
            return pEquipo;
        }
