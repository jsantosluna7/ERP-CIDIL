    ...
    public class RepositorioReservaDeEspacio : IRepositorioReservaDeEspacio
    {
        ...
        public async Task<Resultado<ReservaDeEspacio?>> CrearReserva(CrearReservaDeEspacioDTO crearReservaDeEspacioDTO)
        {
            ...
            if (crearReservaDeEspacioDTO.IdEstado == 1) // Si el estado es "Aprobado"
            {
                ...
                {
                    string nombreLab = laboratorio.Nombre ?? "";
                    await _servicioEmail.EnviarCorreoAprobacion(usuario.CorreoInstitucional, usuario.NombreUsuario, usuario.ApellidoUsuario, nombreLab, fechaFormateadaSolicitud, fechaFormateadaInicio, fechaFormateadaHoraInicio,fechaFormateadaHoraFinal);
                }
            } else if (crearReservaDeEspacioDTO.IdEstado == 3) // Si el estado es "Rechazado"
            {
                ...
                {
                    string nombreLab = laboratorio.Nombre ?? "";
                    string comentario = crearReservaDeEspacioDTO.ComentarioAprobacion ?? "";
                    await _servicioEmail.EnviarCorreoRechazo(usuario.CorreoInstitucional, usuario.NombreUsuario, usuario.ApellidoUsuario, nombreLab, fechaFormateadaSolicitud, fechaFormateadaInicio, comentario, fechaFormateadaHoraInicio, fechaFormateadaHoraFinal);
                }
            }
        ...
        public async Task<Resultado<bool?>> CancelarReserva(int id)
        {
            ...
            //si la reserva es null o no existe, significa que no la encontramos
            if (!reserva.esExitoso || reserva.Valor == null)
            {
        ...
        public async Task<Resultado<bool?>> desactivarReservaDeEspacio(int id)
        {
            ...
            if (!espacioPorId.esExitoso || espacioPorId.Valor == null)
            {
