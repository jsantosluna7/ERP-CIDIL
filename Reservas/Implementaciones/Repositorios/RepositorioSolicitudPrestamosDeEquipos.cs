...
namespace Reservas.Implementaciones.Repositorios
{
    public class RepositorioSolicitudPrestamosDeEquipos : IRepositorioSolicitudPrestamosDeEquipos
    {
        ...
        public async Task<SolicitudPrestamosDeEquipo?> CrearSolicitudPEquipos(CrearSolicitudPrestamosDeEquiposDTO crearSolicitudPrestamosDeEquiposDTO)
        {
            ...
            string cantidadStr = crearReservas.Cantidad?.ToString() ?? "0";

            foreach (var usuarios in usuario)
            {
                await _servicioEmail.EnviarCorreoReservaEquipos(usuarios.CorreoInstitucional, nombreInventario, cantidadStr, fechaFormateadaInicio, fechaFormateadaFinal); //Agregar la url que porque el usuario aprobador podra acceder al id de la solicitud
            }
        ...
        public async Task<SolicitudPrestamosDeEquipo?> ActualizarSolicitudPEquipos(int id, ActualizarSolicitudPrestamosDeEquiposDTO actualizarSolicitudPrestamosDeEquiposDTO)
        {
            ...
            string nombreSolicitante = usuarioSolicitante?.NombreUsuario ?? "";
            string apellidoSolicitante = usuarioSolicitante?.ApellidoUsuario ?? "";

            foreach (var usuarios in usuario)
            {
                await _servicioEmail.EnviarCorreoReservaMoficación(usuarios.CorreoInstitucional, nombreSolicitante, apellidoSolicitante);
            }
